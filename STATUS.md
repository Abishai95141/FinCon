...
| **No breadth control on rules** | The selectivity cap was deleted after a metamorphic relation refuted it. A rule flooding the worklist is unguarded. Any replacement must survive `test_a_rules_verdict_is_invariant_to_padding`. | open |
| **Row-order relation is vacuous** | 0 of 23 anchors on batch A have competing groups, so order-dependence is unfalsifiable here. Guard, not a check. | needs a contested corpus |
